...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Session cap: max 10 user messages per session
    const userMessageCount = messages.filter(m => m.role === 'user').length
    if (userMessageCount > 10) {
      return res.status(200).json({
        content: [{ text: "That's all I've got for one session — if you want to keep the conversation going, reach out directly at hello@ivonnealdaz.com." }]
      })
    }

    // Trim history: only send last 6 messages to the API to keep costs flat
