...
const WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbx-tpWiFWUYVYR47fm4peJeQIRErxnAzdM1Ah4oqoeyNKfMkB0QTE_vI6iTVokwNO3x/exec'
...
async function classifyAndScore(question, response) {
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 60,
        system: 'You classify visitor questions to a personal portfolio site. Respond with ONLY valid JSON, no markdown, no explanation. Format: {"topic":"Art|Brand|Writing|Built|General","leadScore":1}. Lead score 1-5: 1=general curiosity, 3=professional interest, 5=consulting/hiring intent.',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!data.error) {
      ...
      // Classify and log in background — don't block the response
      classifyAndScore(lastQuestion, reply).then(({ topic, leadScore }) => {
        logToSheet(lastQuestion, reply, topic, leadScore)
      })
    }
